...
object BowlingApp2 extends App{
  ...
  //TOTAL = 20
   ...
   var turns = List[Frame]()
    def bowl(pins: Int) {
      if (turns.size > 0 && !turns.last.turnFinished) {
        turns.last.bowl2 = pins
        turns.last.turnFinished = true
      } else {
        turns = turns :+ Frame(pins)
      }
    }

    def score: Int = {
      def sumScore(turnsLeft: List[Frame], total: Int): Int = {
    ...
    case class Frame(bowl1: Int, var bowl2: Int = 0) {
      def strike = bowl1 == 10
